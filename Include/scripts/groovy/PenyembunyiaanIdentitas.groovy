import cucumber.api.java.en.When

import static com.kms.katalon.core.checkpoint.CheckpointFactory.findCheckpoint
import static com.kms.katalon.core.testcase.TestCaseFactory.findTestCase
import static com.kms.katalon.core.testdata.TestDataFactory.findTestData
import static com.kms.katalon.core.testobject.ObjectRepository.findTestObject

import com.kms.katalon.core.annotation.Keyword
import com.kms.katalon.core.checkpoint.Checkpoint
import com.kms.katalon.core.checkpoint.CheckpointFactory
import com.kms.katalon.core.mobile.keyword.MobileBuiltInKeywords as Mobile
import com.kms.katalon.core.model.FailureHandling
import com.kms.katalon.core.testcase.TestCase
import com.kms.katalon.core.testcase.TestCaseFactory
import com.kms.katalon.core.testdata.TestData
import com.kms.katalon.core.testdata.TestDataFactory
import com.kms.katalon.core.testobject.ObjectRepository
import com.kms.katalon.core.testobject.TestObject
import com.kms.katalon.core.webservice.keyword.WSBuiltInKeywords as WS
import com.kms.katalon.core.webui.keyword.WebUiBuiltInKeywords as WebUI

import internal.GlobalVariable

import org.openqa.selenium.WebElement
import org.openqa.selenium.WebDriver
import org.openqa.selenium.By

import com.kms.katalon.core.mobile.keyword.internal.MobileDriverFactory
import com.kms.katalon.core.webui.driver.DriverFactory

import com.kms.katalon.core.testobject.RequestObject
import com.kms.katalon.core.testobject.ResponseObject
import com.kms.katalon.core.testobject.ConditionType
import com.kms.katalon.core.testobject.TestObjectProperty

import com.kms.katalon.core.mobile.helper.MobileElementCommonHelper
import com.kms.katalon.core.util.KeywordUtil

import com.kms.katalon.core.webui.exception.WebElementNotFoundException

import cucumber.api.java.en.And
import cucumber.api.java.en.Given
import cucumber.api.java.en.Then

class PenyembunyianIdentitas {
	/**
	 * The step definitions below match with Katalon sample Gherkin steps
	 */
	@Given("Dosen akses halaman login")
	def halamanLogin() {
		println ("diarahkan ke sistem login")
		WebUI.openBrowser('')
		WebUI.navigateToUrl('http://smart.poltekad.online/web/login')
	}
	@When("Dosen mengisi (.*) dan (.*) yang valid")
	def isiFormLogin(String email, String password) {
		println ("\n Mahasiswa input email dan password")
		println ("email "+email)
		println ("password"+password)
		WebUI.setText(findTestObject('Object Repository/cek_identitas/Page_Login  My Website/input_Email_login'), email)
		WebUI.setEncryptedText(findTestObject('Object Repository/cek_identitas/Page_Login  My Website/input_Kata Sandi_password'), password)
	}
	@And("klik tombol login")
	def tombolLogin() {
		println ("klik button login")
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Login  My Website/button_Log masuk'))
	}

	@Then("Dosen masuk ke halaman sistem")
	def masuk() {
		println ("Dosen diarahkan")
	}

	@Given("Dosen berhasil masuk")
	def sistem() {
		println ("Dosen masuk ke sistem")
	}
	@Then("Dosen memilih menu Penilaian CBT kemudian pilih Dosen")
	def PilihMenu() {
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Diskusi/a_Penilaian CBT'))
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Soal Test/span_Dosen'))
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Soal Test/a_Soal Isian'))
	}
	@And("Terdapat ujian mahasiswa")
	def ujianmhs() {
		println ("Mahasiswa berhasil melihat soal ujian")
	}

	@When("Dosen memilih jawaban dari mahasiswa")
	def pilih() {
		println ("Soal Ujian Tersedia!")
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Penilaian Soal Isian Dan Uraian/div_Soal Uji Coba Sistem (9)'))
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Penilaian Soal Isian Dan Uraian/td_28022024 133159'))
	}


	@And("Identitas mahasiswa tidak boleh terlihat oleh Dosen")
	def IdentitasCek() {
		println ("Identitas mahasiswa tidak terlihat!")
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Soal Uji Coba Sistem/label_Identifikasi tanda'))
		WebUI.click(findTestObject('Object Repository/cek_identitas/Page_Poltekad - Soal Uji Coba Sistem/span_c49d22e6-b73c-4b43-b736-d08e2345db03'))
	}
}



